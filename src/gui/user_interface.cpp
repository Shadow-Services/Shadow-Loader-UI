...
void UI::Render()
{
    ...
    const WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, _T("Shadow Loader"), nullptr };
    ::RegisterClassEx(&wc);
    const HWND hwnd = ::CreateWindow(wc.lpszClassName, _T("Shadow Loader"), WS_OVERLAPPEDWINDOW, 100, 100, 50, 50, NULL, NULL, wc.hInstance, NULL);
    ...
    D3DX11CreateShaderResourceViewFromMemory(pd3dDevice, gta_v_logo, sizeof(gta_v_logo), &image_info,
        NULL, &gtaIcon, 0);
...

    {
        ...
        io.Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(font_storopia), sizeof(font_storopia), 16.f, &fnt_cfg, io.Fonts->GetGlyphRangesDefault());
    }

    {
        ...
        g.window.font_main = io.Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(font_storopia), sizeof(font_storopia), 24.f, &fnt_cfg);
